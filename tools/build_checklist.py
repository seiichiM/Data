...
FLAG = {
 ...
 "cancer":"がん原性物質を取り扱う",
 "noudo":"濃度基準値が定められた物質を取り扱う",
 "ankiken":"労働安全衛生法施行令別表第1の危険物を取り扱う",
 "ban":"製造等が禁止される有害物等に該当するおそれがある",
 "perm":"製造の許可を受けるべき有害物（特化則第1類物質）を取り扱う",
 "muta":"強い変異原性が認められた化学物質を取り扱う",
 "tokka":"特定化学物質を取り扱う",
 "tokka12":"特定化学物質の第1類・第2類物質を取り扱う",
 "kakan1st":"化管法の第一種指定化学物質を取り扱う",
 "leadWork":"鉛業務がある（鉛則第1条第5号の業務）",
 "tokkaS":"特別管理物質を取り扱う",
 "yuki":"有機溶剤等を取り扱う",
 "yuki12":"第1種・第2種有機溶剤等を取り扱う",
 "lead":"鉛になりうる物質を取り扱う",
 "dust":"粉じんになりうる物質を取り扱う",
 "dustWork":"粉じん作業がある（粉じん則別表第1の作業）",
 "measure":"作業環境測定の対象作業場がある",
 "localExhaust":"局所排気装置・プッシュプル型換気装置がある",
 "welding":"金属アーク溶接等の作業がある",
 "confined":"タンク・ピット・暗渠等の内部で作業を行う（酸素欠乏危険場所）",
 "mgmt3":"作業環境測定で第3管理区分となった場所がある",
 ...
 "shOver":"指定数量の倍数が1以上",
 "waste":"産業廃棄物を排出している",
 "spWaste":"特別管理産業廃棄物が生じる（廃棄物そのものの性状で判定するため、台帳では自動判定しません）",
...
TERMS = [
 ...
 (r"製造等が禁止される有害物等|製造等禁止物質", "製造等が禁止される有害物等", "安衛法施行令 第16条",
  "https://laws.e-gov.go.jp/law/347CO0000000318", "e-Gov法令検索 労働安全衛生法施行令"),
 (r"製造の許可を受けるべき有害物|製造許可物質", "製造の許可を受けるべき有害物", "安衛法施行令 第17条・別表第3第1号",
  "https://laws.e-gov.go.jp/law/347CO0000000318", "e-Gov法令検索 労働安全衛生法施行令。特化則の第1類物質と同じ範囲"),
 (r"強い変異原性が認められた化学物質", "強い変異原性が認められた化学物質", "変異原性指針 対象物質",
  "https://anzeninfo.mhlw.go.jp/user/anzen/kag/kagaku_index.html",
  "職場のあんぜんサイト。厚生労働省が公表するたびに追加されます"),
...
<header class="masthead"><div class="wrap mast-in">
  ...
  <div class="mast-meta">
    ...
    <button class="iconbtn" id="btnSave" type="button">HTMLで保存</button>
...
function init(){
  ...
  /* このページ自身をHTMLファイルとして保存する。外部ファイルを参照していないので
     保存したものをそのまま社内で配れる。claude.ai上では downloads 機能を使い、
     それ以外の環境では通常のリンクで保存する。 */
  let canDownload = null;
  if(window.claude && typeof window.claude.use === "function"){
    window.claude.use("downloads").then(d => { if(d) canDownload = d; }).catch(()=>{});
  }
  $("#btnSave").addEventListener("click", async () => {
    const name = "化学物質関係法令_確認点一覧.html";
    const html = ["<!doctype html>", '<html lang="ja">',
                  document.documentElement.innerHTML, "</html>", ""].join(String.fromCharCode(10));
    if(canDownload){
      try{ await canDownload.save({filename:name, data:html}); return; }
